...
if(!empty($old) && !empty($change) && !empty($stud_group) && !empty($teacher) && !empty($lesson) && !empty($module_name) && !empty($semestr)){
	...
	$query = "UPDATE `module_first` SET name_cp = '$change' WHERE stud_group='$stud_group' and teacher='$teacher' and lesson='$lesson' and name_cp='$old' and module_name='$module_name' and semestr='$semestr'";
